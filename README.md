<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:203a43,100:2c5364&height=220&section=header&text=Piyush%20Kumar&fontSize=55&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Software%20Engineer%20%7C%20.NET%20%7C%20Angular%20%7C%20SQL%20Server&descAlignY=58&descSize=18" width="100%"/>

<br>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&duration=3000&pause=1000&color=36BCF7&center=true&vCenter=true&width=700&lines=Hey+%F0%9F%91%8B%2C+I'm+Piyush;Software+Engineer+%7C+.NET+Full+Stack+Developer;Building+APIs+%7C+Web+Applications+%7C+Database+Solutions;Turning+Ideas+into+Production-Ready+Software+%F0%9F%9A%80" alt="Typing SVG" />

<br>

<p>
  <img src="https://komarev.com/ghpvc/?username=piyush-kumar11&label=Profile%20Views&color=0e75b6&style=for-the-badge" alt="Profile Views"/>
  <img src="https://img.shields.io/github/followers/piyush-kumar11?label=Followers&style=for-the-badge&color=0e75b6" alt="GitHub Followers"/>
</p>

</div>

---

## 👨‍💻 About Me

```text
╔════════════════════════════════════════════════════════════╗
║                     SOFTWARE ENGINEER                     ║
╠════════════════════════════════════════════════════════════╣
║  Backend       →  C# / .NET / ASP.NET Core / Web APIs     ║
║  Frontend      →  Angular / JavaScript / HTML / CSS       ║
║  Database      →  SQL Server / MySQL / Stored Procedures ║
║  Data Access   →  Dapper / ADO.NET                       ║
║  Tools         →  Git / Postman / Swagger / Docker       ║
║  Architecture  →  REST APIs / Integrations / Services    ║
╚════════════════════════════════════════════════════════════╝
```

I'm a **Software Engineer focused on building reliable, scalable and production-ready applications**.

I enjoy working across the stack — from designing APIs and database logic to building clean frontend experiences.

### 🚀 What I Do

* 🔹 Build **ASP.NET Core Web APIs**
* 🔹 Design and optimize **SQL Server queries & Stored Procedures**
* 🔹 Develop **Angular applications**
* 🔹 Work with **Dapper & data-access layers**
* 🔹 Debug and troubleshoot production issues
* 🔹 Build integrations between systems and APIs
* 🔹 Work with authentication, APIs and backend services
* 🔹 Continuously learn new technologies and engineering practices

---

## 🛠️ Tech Stack

### 💻 Backend

<p align="center">
<img src="https://skillicons.dev/icons?i=cs,dotnet,java,spring" />
</p>

### 🌐 Frontend

<p align="center">
<img src="https://skillicons.dev/icons?i=angular,html,css,js" />
</p>

### 🗄️ Database & Data

<p align="center">
<img src="https://skillicons.dev/icons?i=mysql" />
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/microsoftsqlserver/microsoftsqlserver-plain.svg" width="48" height="48" alt="SQL Server"/>
</p>

### ⚙️ Tools & Platforms

<p align="center">
<img src="https://skillicons.dev/icons?i=git,github,docker,postman,python,arduino" />
</p>

---

## 🔥 Currently Exploring

<p align="center">

<img src="https://img.shields.io/badge/Azure-Cloud%20Development-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white"/>
<img src="https://img.shields.io/badge/Angular-Advanced-DD0031?style=for-the-badge&logo=angular&logoColor=white"/>
<img src="https://img.shields.io/badge/.NET-Backend-512BD4?style=for-the-badge&logo=dotnet&logoColor=white"/>
<img src="https://img.shields.io/badge/System%20Design-Learning-111827?style=for-the-badge&logo=diagramsdotnet&logoColor=white"/>

</p>

---

## 🚀 Featured Project

### 🏠 HomeCheck

A personal mobile project focused on location-based home arrival/departure notifications.

**Goal:** Build a practical application while learning modern mobile development and system integration.

```text
📱 Mobile App
     │
     ├── 📍 Location Detection
     │
     ├── 🏠 Home Geofence
     │
     ├── 🔔 Event Detection
     │
     └── 📩 Notification
```

> Building projects isn't just about writing code — it's about solving real problems.

---

## 📊 GitHub Analytics

<div align="center">

<a href="https://github.com/Piyush-Kumar11">
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=Piyush-Kumar11&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&include_all_commits=true&cache_seconds=1800" alt="Piyush's GitHub Stats"/>
</a>

<a href="https://github.com/Piyush-Kumar11">
  <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Piyush-Kumar11&layout=compact&theme=tokyonight&hide_border=true&langs_count=8&cache_seconds=1800" alt="Piyush's Top Languages"/>
</a>

</div>

<br>

<div align="center">

<img src="https://streak-stats.demolab.com?user=Piyush-Kumar11&theme=tokyonight&hide_border=true&border_radius=10&cache_seconds=1800" alt="Piyush's GitHub Streak"/>

</div>

---

## 🐍 Contribution Journey

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Piyush-Kumar11/Piyush-Kumar11/output/github-contribution-grid-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Piyush-Kumar11/Piyush-Kumar11/output/github-contribution-grid-snake.svg">
  <img src="https://raw.githubusercontent.com/Piyush-Kumar11/Piyush-Kumar11/output/github-contribution-grid-snake.svg" alt="GitHub Contribution Snake" width="100%">
</picture>

</div>

---

## 🏆 GitHub Achievements

<div align="center">

<a href="https://github.com/Piyush-Kumar11">

<img src="https://github-profile-trophy.vercel.app/?username=Piyush-Kumar11&theme=tokyonight&no-frame=true&no-bg=true&margin-w=8&margin-h=8&row=1&column=7" alt="Piyush's GitHub Trophies"/>

</a>

</div>

---

## 🌐 Connect With Me

<div align="center">

<a href="https://linkedin.com/in/piyush2tiger">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://medium.com/@piyushkumar41214">
<img src="https://img.shields.io/badge/Medium-Read%20My%20Articles-000000?style=for-the-badge&logo=medium&logoColor=white"/>
</a>

<a href="https://leetcode.com/a1piyush">
<img src="https://img.shields.io/badge/LeetCode-Solve%20Problems-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>
</a>

<a href="https://www.hackerrank.com/piyush2t">
<img src="https://img.shields.io/badge/HackerRank-Profile-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white"/>
</a>

<a href="https://www.codechef.com/users/piyush2kumar">
<img src="https://img.shields.io/badge/CodeChef-Profile-5B4638?style=for-the-badge&logo=codechef&logoColor=white"/>
</a>

<a href="https://auth.geeksforgeeks.org/user/piushverma111">
<img src="https://img.shields.io/badge/GeeksforGeeks-Learn-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white"/>
</a>

</div>

---

## 💭 Developer Mindset

<div align="center">

### *"Turning concepts into reality — one line of code at a time."* 🚀

<br>

```text
Learn → Build → Break → Debug → Improve → Repeat 🔄
```

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2c5364,50:203a43,100:0f2027&height=120&section=footer" width="100%"/>

</div>
